...
class TestUsersDAO:
    ...
    @pytest.mark.parametrize('email', ['TestUser1@test.ru', 'TestUser2@test.ru'])
    def test_get_email(self, users_dao, user_1, user_2, email):
        user = users_dao.get_email(email)
        assert user.email is not None
        assert user.email == email

    def test_update_info(self, users_dao, user_1, user_update):
        assert users_dao.update_info(user_update, user_1.email) is None
